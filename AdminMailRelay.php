...
class AdminMailRelay extends ModuleAdminController {
	...
	public function display() {
		...
		if ($is_post) {
			if ('save_credential' == $params->mailrelay_option) {
				...
				if ($result) {
					...
					$params->mailrelay_username = $result ['username'];
					$params->mailrelay_password = $result ['password'];
				}
		...
		if ($hasCredential) {
			...
			$params->mailrelay_username = $credentials ['username'];
			$params->mailrelay_password = $credentials ['password'];
		}
	...
	protected function _getCredentials() {
		...
		$sql = "SELECT *, aes_decrypt(unhex(`password`), `hostname`) as `password` FROM `{$db_prefix}mailrelay` LIMIT 1";
	...
	protected function _saveCredential(Zend_Config $data) {
		...
		$username = Db::getInstance()->escape($data->mailrelay_username);
		$password = Db::getInstance()->escape($data->mailrelay_password);
		...
		try {
			...
			if (is_array ( $result )) {
				...
				$this->_showMessage ( $this->_('please_verify_the_hostname'), 'error' );
				return false;
			}
